...
def save_model(trained_model, num_epochs, batch_size, learning_rate):
    '''Saves the trained model, training loss, and validation loss and how 
    it evolves over the epochs'''
    
    info = str(num_epochs)+'E-'+str(batch_size)+'B-'+str(learning_rate)+'L'
    torch.save(trained_model.state_dict(), 'CAE_'+info+'.pt')
    print('\nModel and training history saved successfully.\n')

save_model(cae, 
           config['trainer_params']['max_nb_epochs'],
